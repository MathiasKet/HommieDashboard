      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 border-t border-border flex-shrink-0 space-y-2">
            {(user || (typeof window !== 'undefined' && localStorage.getItem('hommie_admin:session'))) && (
              <div className="px-4 py-3 flex items-center gap-3 bg-background rounded-2xl mb-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-text-primary truncate">
                    {user?.fullName || (typeof window !== 'undefined' && JSON.parse(localStorage.getItem('hommie_admin:admins') || '[]').find((a: any) => a.id === JSON.parse(localStorage.getItem('hommie_admin:session') || '{}').adminId)?.full_name) || 'Admin User'}
                  </p>
                  <p className="text-xs text-text-secondary truncate">
                    {user?.primaryEmailAddress?.emailAddress || (typeof window !== 'undefined' && JSON.parse(localStorage.getItem('hommie_admin:admins') || '[]').find((a: any) => a.id === JSON.parse(localStorage.getItem('hommie_admin:session') || '{}').adminId)?.email) || 'admin@hommie.com'}
                  </p>
                </div>
              </div>
            )}
