...
function ClerkAdminLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const admin = mockLogin(email, password);
      
      if (admin) {
        // Since we bypassed clerkMiddleware, we can just redirect
        router.push('/dashboard');
      } else {
        setError('Login failed. Please check your credentials or use the demo accounts.');
      }
    } catch (err: any) {
